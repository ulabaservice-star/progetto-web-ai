...
| `remove-chat` (OGW-601) | **done — mergiato `main` (`0bf3512`, ff-only, deploy)** | **4/4 VERDE** (mut 3/3) | `wizard-shell` |

**Build order (DAG):** `{ai-usage-guard, offerings-editor} → generate-description · suggest-offerings → wizard-shell → remove-chat`.

## 2. Macrotask corrente

- **✅ WORKSTREAM `onboarding-guided-wizard` COMPLETO — DAG CHIUSO (6/6).** `remove-chat` (OGW-601) è l’ultimo nodo: nessun macrotask successivo. Prossima priorità FUORI da questo workstream (roadmap): polish estetico / P5 billing. La storia sotto resta per riferimento.
...
| Campo | Valore |
|---|---|
| Branch di lavoro | `trueline/build/remove-chat` (OGW-601 `0bf3512`), mergiato ff-only su `main`. (Prec.: `trueline/build/wizard-shell`.) |
| Ultimo commit su `main` | `0bf3512` (feat remove-chat OGW-601) + docs(session-end). **DAG CHIUSO: workstream completo.** |
| Stato merge su `main` | **MERGIATO ff-only su `main` (`0bf3512`), push → deploy Vercel partito (2026-08-25)** — gate umano APPROVATO («merge a main ora»; nessun gate visivo, macrotask di sola rimozione). Verifica locale VERDE PRIMA del merge: tsc 0, eslint 0, knip 0, suite **1744**, `next build` 0 (/turn assente dal manifest), checkpoint C1/C2 verde (C1 `dup:223` 0-nuove + ri-baseline igiene 225→219 legittima, misura diretta per-file: 0 cloni toccano il macrotask), e2e Chromium 2/2, mutazioni 3/3. |
...
### Lezioni build M7 `remove-chat` OGW-601 (2026-08-25)

- **`runOnboardingTurn` NON è della chat** — è il confine LLM unico (`onboardingLlmPort` in `data/llm-ports.ts`), consumato da generate-description/suggest-offerings. Sparisce solo `runInterviewTurn` (interview.ts). Prima di rimuovere: i due file domain che citano `interview` (`generation/document.ts`, `onboarding/generate-description.ts`) lo fanno **solo in commenti**, non in import.
- **Test misti = split, non cancellazione in blocco.** `onboarding-route`/`brief-length-caps` mescolavano test-endpoint-chat e test-invarianti (pagina T-150, cap P1-D17, cross-tenant P1-D21 sulla pagina): rimossi i primi, tenuti i secondi. La copertura sicurezza endpoint (same-origin/byte/cross-tenant) resta sugli endpoint AI superstiti.
- **Liste di path rappresentative → sostituire, non svuotare.** `anthropic-boundary`/`supabase-clients` usano un path del layer domain come rappresentante per lintare una **fixture-stringa** (non leggono il file): `interview.ts` → `generate-description.ts`/`brief.ts` (moduli domain/onboarding superstiti). Svuotare l'entry avrebbe tolto il rappresentante del layer (in PERCORSI_LIBERI_LLM è "il PRIMO, non un dettaglio").
- **eslint no-unused-vars = guida agli orfani a cascata.** Rimosso il describe endpoint-chat, gli orfani emergono a catena (fixture confine → helper turn → import): rimosso il wiring ovvio, poi eslint ha dato la lista esatta invece di indovinare. **I `.test.ts` sono ESCLUSI da jscpd** (i due target test nuovi non entrano nel gate igiene).
- **Ri-baseline igiene 225→219 LEGITTIMA per riorganizzazione cluster.** Rimuovendo `turn/route.ts` (nel cluster `jsonError`/wizard) jscpd ricalcola i cluster residui → 4 fingerprint "falsi-nuovi" su file **NON toccati** (WizardNav/route-guards/StepContacts/OfferingSuggestions). **Misura diretta jscpd@4** (regola M4/M6): 0 cloni coinvolgono un file del macrotask (l'unico `.tsx` di produzione toccato, `OnboardingWorkspace`, è solo un commento e non appare in alcun clone) → ratchet legittimo (dup 226→223, l'igiene MIGLIORA), non mascheramento. Metodo M2: fingerprint da `control1Hygiene(baseline vuota).findings`, mai `capture --hygiene`.
- **AC-601-3 characterization via `resolveVariantHome`.** Il regression test pinna il documento INTERO (`toMatchSnapshot`) da `selectDesign`+`resolveVariantHome` con brief/pool/seed fissi (snapshot PRE-rimozione). Mutazione motore (variantIndex+1) → snapshot rosso: ancorato al motore reale, non vacuo.

## 6. Copertura dichiarata

- **`remove-chat` (OGW-601)** — target_tests coperti: `tests/onboarding-chat-removed.test.ts` (AC-601-1 rotta /turn + cartella turn/ assenti; AC-601-2 interview.ts assente + nessun sorgente di produzione importa interview/ChatPanel/turn, scansione statica dell'albero) + `tests/onboarding-generation-regression.test.ts` (AC-601-3 documento pinnato via selectDesign+resolveVariantHome con brief/pool/seed fissi, characterization pre-rimozione). **Mutazioni 3/3 UCCISE**: ricreare turn/route.ts ⇒ AC-601-1 rosso; import interview iniettato ⇒ AC-601-2 rosso; motore mutato (variantIndex+1) ⇒ snapshot AC-601-3 rosso. Checkpoint decomposto 4/4 verde. Nessun gate visivo (sola rimozione). `runOnboardingTurn`/Brief/motore INVARIATI.
