# session-end — Belora/Ulaba · design-engine (motore visivo di generazione)
...
```
Chiudiamo la sessione su **design-engine**. Aggiorna la fonte di verità dello stato vivo.
...
1) CHECKPOINT — conferma che, per ogni macrotask toccato in questa sessione, run_checkpoint.mjs è
   ...
   NON dichiararlo done: registra lo stato reale (in_progress) e cosa manca.

2) AGGIORNA docs/blueprint/design-engine/SESSION-STATE.md:
   • §1 Stato dei macrotask: visual-skin / design-select / effects-runtime / e2e-visual →
     todo | in_progress | done, con il commit del checkpoint e una riga di note (task coperti, esito
     4/4, igiene, eventuali FP baselinati).
   ...
   • §6 Copertura dichiarata: quali AC/target_tests sono passati; cosa NON è coperto (dichiaralo, non
     riempirlo con una stima — L-COL-006).
